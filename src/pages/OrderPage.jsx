...
function discountSummary(discounts = []) {
  return discounts.map((discount) => discount.name).filter(Boolean).join('、')
}
...
export default function OrderPage({ role: roleProp, adminSession }) {
  ...
  useEffect(() => {
    ...
    async function loadStoreProductStatuses() {
      ...
      const statuses = await storeProductStatusService.listStoreProductStatuses({ storeId: selectedStoreId, date: getStoreStatusDate() })
  ...
  const storeVisibleProducts = useMemo(() => {
    return products
      .map((product) => {
        ...
        return { ...product, storeStatus, isSoldOut: storeStatus === PRODUCT_STORE_STATUS.SOLD_OUT }
      })
  ...
  async function saveProfile(event) {
    ...
    if (!isStore && lineProfile?.userId) {
      await customerProfileService.save({ ...nextProfile, lineUserId: lineProfile.userId, lineDisplayName: lineProfile.displayName || '', linePictureUrl: lineProfile.pictureUrl || '' }).catch((error) => setMessage(error.message || '顧客資料儲存失敗，請稍後再試。'))
    }
  }
  ...
  function toggleDiscount(discount) {
    setSelectedDiscountIds((current) => current.includes(discount.id) ? current.filter((id) => id !== discount.id) : [...current, discount.id])
  }

  function clearCartForStoreChange(nextStoreId) {
  ...
  function renderLineWarning(extraClassName = '') {
    ...
    return <p className={`rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-semibold leading-6 text-red-700 ${extraClassName}`}>未使用 LINE 點餐，將無法收到店家接單、取消等訂單狀態通知。請務必留下可聯絡電話。</p>
  }
    ...
    return (
      <div className="mx-auto max-w-xl px-4 py-10">
        <section className="card border-accent/30 bg-green-50/80 p-7 text-center">
          ...
          <button className="btn-primary mt-6 w-full" type="button" onClick={connectLineProfile} disabled={lineChecking || (!env.useMockData && !env.isLiffEnabled)}>{lineChecking ? '正在確認 LINE 身份...' : '使用 LINE 開始點餐'}</button>
        ...
        <section className="mt-4 rounded-3xl border border-line bg-white/80 p-4">
          <button className="flex w-full items-center justify-between text-left text-sm font-black text-muted" type="button" onClick={() => setNoLineExpanded(!noLineExpanded)}><span>沒有 LINE，也可以繼續點餐</span><ChevronDown className={`transition ${noLineExpanded ? 'rotate-180' : ''}`} size={18} /></button>
          {noLineExpanded && <div className="mt-4 space-y-3">{renderLineWarning()}<button className="btn-secondary w-full" type="button" onClick={() => setAllowNoLineOrder(true)}>我了解，仍要繼續點餐</button></div>}
        </section>
      </div>
    )
  }
  ...
  async function submitOrder() {
    ...
    const order = await orderService.createOrder({
      ...
      note: [
        ...
        selectedDiscounts.length ? `折扣：${discountSummary(selectedDiscounts)}，折抵 ${formatPrice(discountAmount)}` : '',
  ...
  function renderStoreSelector() {
    ...
    return (
      <section className="card p-4">
        ...
        <select className="input mt-3" value={selectedStoreId} onChange={(event) => clearCartForStoreChange(event.target.value)} disabled={role === 'store' && !!adminSession?.storeId}>{activeStores.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select>
    ...
    return (
      <div className="space-y-3">
        ...
        <section className={isStore ? 'card p-3' : 'card p-4'}>
          ...
          {diningType === 'delivery' && <div className="mt-3 space-y-2"><label className="block space-y-1"><span className="label">外送地址 *</span><textarea className="input min-h-24" value={deliveryAddress} onChange={(event) => setDeliveryAddress(event.target.value)} placeholder="請填寫外送地址、樓層或備註" autoComplete="street-address" /></label><div className={`rounded-2xl p-3 text-xs leading-5 ${isOverDeliveryDistance ? 'bg-red-50 text-red-700' : 'bg-cream text-muted'}`}>{maxDeliveryDistanceKm > 0 && <p>可外送距離：{maxDeliveryDistanceKm} km 內{selectedStoreDistanceKm !== null ? `｜目前約 ${selectedStoreDistanceKm.toFixed(1)} km` : ''}</p>}{freeDeliveryMinAmount > 0 && <p>滿 {formatPrice(freeDeliveryMinAmount)} 免費外送｜目前 {cartTotal >= freeDeliveryMinAmount ? '已達門檻' : `還差 ${formatPrice(freeDeliveryMinAmount - cartTotal)}`}</p>}{maxDeliveryDistanceKm === 0 && freeDeliveryMinAmount === 0 && <p>外送條件未設定，請依門店實際規則確認。</p>}</div></div>}
        ...
        <section className={isStore ? 'card p-3' : 'card p-4'}>
          ...
          {timeType === 'scheduled' && <div className="mt-3 grid gap-2"><label className="space-y-1"><span className="label">日期 *</span><input className="input" type="date" min={minScheduledDate} value={orderDate} onChange={(event) => setOrderDate(event.target.value)} /></label><label className="space-y-1"><span className="label">時間 *</span><input className="input" type="time" value={orderTime} onChange={(event) => setOrderTime(event.target.value)} /></label>{timeSettings.preorderMinDays > 0 && <p className="text-xs text-muted">需至少提前 {timeSettings.preorderMinDays} 天預定。</p>}</div>}
        </section>
      </div>
    )
  }
  ...
  function renderCustomerStoreHeader() {
    ...
    return (
      <div>
        ...
        <div className="relative rounded-2xl bg-cream px-3 py-2 pr-10"><select id="customer-store-select" className="w-full appearance-none bg-transparent text-base font-black text-ink outline-none" value={selectedStoreId} onChange={(event) => clearCartForStoreChange(event.target.value)}><option value="" disabled>下拉選擇門店</option>{activeStores.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-brand" size={18} /></div>
  ...
  if (!isStore && !hasProfile) {
    return (
      <div className="mx-auto max-w-xl px-4 py-10"><form className="card p-7" onSubmit={saveProfile}><UserRound className="text-accent" size={36} /><p className="text-xs font-semibold text-accent">First Order Profile</p><h1 className="mt-1 text-3xl font-black">首次訂餐資料</h1><p className="mt-3 text-sm leading-6 text-muted">第一次訂餐需要留下姓名與電話。LINE 會作為主要通知方式；電話只限訂單聯絡使用，當 LINE 聯絡不上你時，店家才會撥打電話。</p>{isLineLoggedIn && <p className="mt-4 rounded-2xl bg-green-50 p-3 text-sm font-semibold text-green-700">LINE 已連結：{lineProfile.displayName || '顧客'}</p>}{allowNoLineOrder && renderLineWarning('mt-4')}<div className="mt-5 space-y-3"><label className="block space-y-1"><span className="label">姓名 *</span><input className="input" value={profileDraft.name} onChange={(event) => setProfileDraft({ ...profileDraft, name: event.target.value })} required /></label><label className="block space-y-1"><span className="label">電話 *</span><input className="input" value={profileDraft.phone} onChange={(event) => setProfileDraft({ ...profileDraft, phone: event.target.value })} required inputMode="tel" /></label></div><p className="mt-3 rounded-2xl bg-cream p-3 text-xs leading-5 text-muted">儲存後，下次使用同一個 LINE 身份點餐時，系統會自動帶入資料，不需要每次重填。</p>{message && <p className="mt-4 rounded-2xl bg-red-50 p-3 text-sm font-semibold text-red-700">{message}</p>}<button className="btn-primary mt-5 w-full" type="submit">儲存並開始點餐</button></form></div>
    )
  }

  if (successOrder) {
    return <div className="mx-auto max-w-2xl px-4 py-10"><div className="card p-8 text-center"><CheckCircle2 className="mx-auto text-accent" size={52} /><h1 className="mt-4 text-3xl font-black">訂餐成功</h1><p className="mt-2 text-muted">訂單編號：{successOrder.orderNumber}</p><p className="mt-4 text-4xl font-black text-brand">{formatPrice(successOrder.totalAmount)}</p>{Number(successOrder.discountAmount || 0) > 0 && <p className="mt-2 text-sm font-semibold text-red-600">已折抵 {formatPrice(successOrder.discountAmount)}</p>}{!isLineLoggedIn && <p className="mt-4 rounded-2xl bg-red-50 p-3 text-sm font-semibold leading-6 text-red-700">你尚未使用 LINE 點餐，本次訂單不會收到接單或取消等狀態通知。請留意電話聯絡。</p>}<button className="btn-primary mt-6" type="button" onClick={() => setSuccessOrder(null)}>建立下一筆訂單</button></div></div>
  }

  if (!isStore && checkoutStep === 'checkout') {
    return (
      <div className="mx-auto grid max-w-5xl gap-6 px-4 py-6 lg:grid-cols-[1fr_360px]">
        <main className="space-y-5"><section className="card p-5"><p className="text-xs font-semibold text-accent">Checkout</p><h1 className="mt-1 text-3xl font-black">確認訂單</h1><p className="mt-2 text-sm text-muted">確認門店、用餐方式、用餐 / 取餐 / 外送時間與訂單聯絡資料。</p>{isLineLoggedIn && <p className="mt-4 rounded-2xl bg-green-50 p-3 text-sm font-semibold text-green-700">LINE 通知將傳送給：{lineProfile.displayName || profile.name}</p>}{allowNoLineOrder && renderLineWarning('mt-4')}</section>{renderOrderOptionsPanel()}<section className="card p-5"><h2 className="font-black">訂單聯絡資料</h2><div className="mt-3 rounded-2xl bg-cream p-4 text-sm"><p className="font-bold">{profile.name}</p><p className="mt-1 text-muted">{profile.phone}</p></div><label className="mt-4 block space-y-1"><span className="label">訂單備註</span><textarea className="input min-h-24" placeholder="例如：餐具需求、特殊備註" value={note} onChange={(event) => setNote(event.target.value)} /></label></section>{message && <p className="rounded-2xl bg-red-50 p-3 text-sm font-semibold text-red-700">{message}</p>}<button className="btn-secondary" type="button" onClick={() => setCheckoutStep('ordering')}>返回修改商品</button></main>
        <div className="space-y-4"><CartPanel items={cartItems} discounts={discountOptions} selectedDiscounts={selectedDiscounts} onToggleDiscount={toggleDiscount} onRemove={removeFromCart} onSubmit={submitOrder} submitLabel="送出訂單" /></div>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-0 py-0 pb-32 md:px-3 md:py-4 md:pb-4">
      <main className={isStore ? 'space-y-3 px-3 md:grid md:grid-cols-[1fr_340px] md:gap-4 md:px-0' : 'space-y-0'}>
        {!isStore && <section className="grid h-[calc(100vh-73px)] grid-cols-[104px_1fr] overflow-hidden bg-white lg:h-[calc(100vh-81px)] lg:rounded-3xl lg:border lg:border-line"><aside className="overflow-y-auto border-r border-line bg-white pb-28"><div className="sticky top-0 z-10 bg-white px-3 py-4 text-base font-black text-ink">熱銷</div><div className="space-y-0.5 pb-4">{categories.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={`w-full border-l-2 px-3 py-4 text-left text-sm font-semibold leading-6 transition ${category === item ? 'border-brand bg-cream text-ink' : 'border-transparent text-muted'}`}>{item}</button>)}</div></aside><section className="overflow-y-auto pb-32"><div className="sticky top-0 z-10 border-b border-line bg-white/95 px-3 py-3 backdrop-blur">{renderCustomerStoreHeader()}</div><div className="grid grid-cols-2 gap-2 px-2 py-2.5 sm:gap-2.5 sm:px-3 md:grid-cols-3 xl:grid-cols-4">{visibleProducts.map((product) => <ProductCard key={product.id} product={product} onSelect={setSelectedProduct} compact dense />)}{visibleProducts.length === 0 && <p className="col-span-2 p-8 text-center text-sm text-muted md:col-span-3 xl:col-span-4">這間門店今日尚未上架此分類商品。</p>}</div></section></section>}
        {isStore && <section className="space-y-3"><section className="card p-4"><p className="text-xs font-semibold text-accent">Counter Order</p><h1 className="mt-1 text-2xl font-black">門店櫃檯點餐</h1><p className="mt-2 text-sm text-muted">目前門店：{selectedStore?.name || '未選擇'}</p></section><section className="card p-3"><div className="mb-3 flex items-center gap-2"><ShoppingBag size={18} className="text-accent" /><h2 className="font-black">商品</h2></div><div className="grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-4">{visibleProducts.map((product) => <ProductCard key={product.id} product={product} onSelect={setSelectedProduct} compact dense />)}{visibleProducts.length === 0 && <p className="col-span-2 p-8 text-center text-sm text-muted md:col-span-3 xl:col-span-4">這間門店今日尚未上架此分類商品。</p>}</div></section></section>}
        <div className={isStore ? 'space-y-3 md:sticky md:top-24 md:self-start' : 'fixed inset-x-0 bottom-0 z-30 border-t border-line bg-cream/95 p-3 backdrop-blur lg:static lg:border-0 lg:bg-transparent lg:p-0'}>
          ...
          <CartPanel items={cartItems} discounts={discountOptions} selectedDiscounts={selectedDiscounts} onToggleDiscount={toggleDiscount} onRemove={removeFromCart} onSubmit={goCheckout} submitLabel={isStore ? '建立櫃檯訂單' : '前往結帳'} compact={!isStore} />
